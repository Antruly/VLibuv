...
void VLogger::setEnable(bool eb){
    enable = eb; }
void VLogger::setFastLog(bool fslog){
	fast_log = fslog;
}
...
void VLogger::fastLog(const std::string& filepath,
                  std::ofstream& logfile,
                  const std::string& level,
                  const std::string& message,
                  const std::string& colour,
  ...
  logMessage =
      getDate(time_now) + " " + getTime(time_now) + " " + level + " " + message;
  VString vlogMessage;;
  if (logMessage.size()>100)
  {
	  vlogMessage = logMessage;
	  std::string  showlogMessage;
	 if (logMessage.size()>500)
	 {
		 showlogMessage = logMessage.substr(0, 500) + "...";
	 }
	 else
	 {
		 showlogMessage = logMessage;
	 }
	 std::cout << colour << showlogMessage.c_str()
		  << VCORE_ANSI_COLOR_RESET
		  << std::endl;
  }
  else{
	  vlogMessage = ((VString)logMessage).toString(log_encoding);
	  std::cout << colour << vlogMessage.c_str()
		  << VCORE_ANSI_COLOR_RESET
		  << std::endl;
  }
